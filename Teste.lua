...
MainFrame.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        if pos.X < framePos.X + borderSize or pos.X > framePos.X + frameSize.X - borderSize or
           pos.Y < framePos.Y + borderSize or pos.Y > framePos.Y + frameSize.Y - borderSize then
            Dragging = true
...
local function AnimateTitleColor()
    while true do
        ...
        for i = 1, #colors - 1 do
            local tween = TweenService:Create(TitleLabel, tweenInfo, {TextColor3 = colors[i + 1]})
            tween:Play()
...
coroutine.wrap(AnimateTitleColor)()
